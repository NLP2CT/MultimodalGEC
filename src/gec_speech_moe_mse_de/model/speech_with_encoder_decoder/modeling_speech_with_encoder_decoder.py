...
class MultiModalLMOutput(ModelOutput):
    ...
    loss_moegate: Optional[torch.FloatTensor] = None
...
class JointSpeechTextEncoder(PreTrainedModel):
    ...
    def forward(
        ...
        text_vec_mean = torch.mean(text_encoder_hidden, dim=1) # bs * hidden
        att_feats_mean = torch.mean(speech_encoder_hidden, dim=1)
        att_feats_mean, gate_loss = self.speech_MoE(att_feats_mean)
        # target_vec_mean = torch.mean(text_vec_mean, dim=1) # bs
        

        hcl_loss = self.mseloss(att_feats_mean, text_vec_mean)

        return MultiModalLMOutput(
            last_hidden_state=encoder_hidden_states,
            loss_contrastive=hcl_loss,
            loss_moegate=gate_loss
        ...
        **kwargs,
        ...
        if encoder_outputs is None:
            ...
            encoder_outputs = BaseModelOutput(
                ...
                hidden_states=encoder_outputs['hidden_states'] if len(encoder_outputs) > 3 else None,
                attentions=encoder_outputs['attentions'] if len(encoder_outputs) > 4 else None,
            )
        ...
        if labels is not None:
            ...
            loss_moegate = encoder_outputs['loss_moegate']

            loss = loss + 0.2 * loss_contrastive + 0.1 * loss_moegate
